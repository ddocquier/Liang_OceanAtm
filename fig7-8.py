...
# File names
filename_liang = dir_output + 'SST_THF_Liang_rm' + str(time_rm) + '_01.npy'
filename_sig = dir_output + 'SST_THF_Liang_rm' + str(time_rm) + '_sig_fdr_alpha005.npy'
...
if save_var == True:
    ...
    for i in np.arange(n_iter-1):
        print(i)
        if i < 8:
            filename = dir_output + 'SST_THF_Liang_rm' + str(time_rm) + '_0' + str(i+2) + '.npy'
        else:
            filename = dir_output + 'SST_THF_Liang_rm' + str(time_rm) + '_' + str(i+2) + '.npy'
...
# Fig. 7: Relative transfer of information SST-THF
...
ax1.set_title(r'$\tau_{SST \longrightarrow THF}$ 1988-2017 (' + str(time_rm) + '-month mean) - J-OFURO3',fontsize=24)
...
gl.ylocator = mticker.FixedLocator(np.arange(-90,86,30))
gl.yformatter = LATITUDE_FORMATTER
ax2.set_title(r'$\tau_{THF \longrightarrow SST}$ 1988-2017 (' + str(time_rm) + '-month mean) - J-OFURO3',fontsize=24)
