...
# File names
if boot_iter < 10:
    filename_liang = dir_output + 'SST_THF_Liang_2var_0' + str(boot_iter) + '.npy'
else:
    filename_liang = dir_output + 'SST_THF_Liang_2var_' + str(boot_iter) + '.npy'
...
# Remove trend and seasonality of SST and THF and compute Liang index in each grid point
for y in np.arange(ny):
    ...
    for x in np.arange(nx):
        if np.count_nonzero(np.isnan(sst[:,y,x])) >= 1 or np.count_nonzero(np.isnan(thf[:,y,x])) >= 1:
            ...
            xx = np.array((sst_resid,thf_resid))
