...
'''
GOAL
    Compute Liang index including 4 variables: SST, SSTt, THF, THF(-1)
    With a lag in THF
...
# File names
if boot_iter < 10:
    filename_liang = dir_output + 'SST_THF_Liang_lag' + str(shift) + '_4var_0' + str(boot_iter) + '.npy'
else:
    filename_liang = dir_output + 'SST_THF_Liang_lag' + str(shift) + '_4var_' + str(boot_iter) + '.npy'
...
# Shift THF (to take lag into account)
thf_lag = np.roll(thf,shift,axis=0)
...
for y in np.arange(ny):
    ...
    for x in np.arange(nx):
        if np.count_nonzero(np.isnan(sst[:,y,x])) >= 1 or np.count_nonzero(np.isnan(thf[:,y,x])) >= 1:
            ...
            thf_lag_resid = seasonal_decompose(thf_lag[:,y,x],model='additive',period=12,extrapolate_trend='freq').resid
            xx = np.array((sst_resid,sst_trend_resid,thf_resid,thf_lag_resid))
